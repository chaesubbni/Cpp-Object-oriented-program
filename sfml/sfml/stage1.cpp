	...
	gbl::Map<>& _map)
{
	for (unsigned short a = 0; a < gbl::MAP::COLUMNS; a++)
	{
		for (unsigned short b = 0; b < gbl::MAP::ROWS; b++)
		{
			...
			{
				_map[a][b] = gbl::MAP::Cell::Start; // 만약 start_position이면, 해당 맵의 Cell 타입을 Start로 바꿈.
			}


			else if (a == _finish_position.first && b == _finish_position.second)
			{
				_map[a][b] = gbl::MAP::Cell::Finish; // 만약 finish_position이면, 해당 맵의 Cell 타입을 Finish로 바꿈.
			}
